...
> ***Revision:*** add `warning: false` to doc YAML options to suppress package loading messages & others
...
> ***Revisions***: (q4 - q7)
>
> -   Move definition of aesthetic param for single layer graphs to ggplot call (global def.)
>
>     -   Makes adding new geom layers less of a hassle
>
> -   Change rat to rodent in labels
>
> -   Remove unnecessary `surveys$` when defining plot axis variables
>
>     -   ggplot already knows what dataset to access thanks to defining `data = surveys`
...
ggplot(data = surveys, mapping = aes(x = weight, y = species)) +
  ...
  geom_jitter(width = 0.15, height = 0.15, color = "cadetblue", alpha = 0.15) +
...
ggplot(data = surveys, mapping = aes(x = weight, y = species)) +
  geom_jitter(width = 0.15, height = 0.15, color = "darkseagreen", alpha = 0.15) +
  labs(x = "Weight of Rodent (g)", y = "Species of Rodent")
```
...
> ***Revisions:***
>
> -   Swap x & y labels to match swapped axis variables
>
> -   Rotate and position species labels to make them all readable and eliminate overlapping
>
> -   Use labs() instead of xlab & ylab for efficiency

```{r}
# Code for question 14!
ggplot(data = surveys, mapping = aes(y = weight, x = species)) +
  geom_boxplot(outlier.alpha = 0) +
  geom_jitter(width = 0.15, height = 0.1, color = "tomato", alpha = 0.15) +
